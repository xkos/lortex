...
兼具**模块化、高性能的 Rust Agent 框架库**与**开箱即用的本地 LLM Proxy 服务**（二进制）双重形态，用于构建 AI 助手、自动化工具和智能分发网关。
...
| 6 | 不同的 AI Coding 软件（Cursor/Windsurf等）需要各自配置和切换大模型，缺乏统一聚合管理 | 缺乏本地统一的 LLM 代理网关 |
...
#### 需求 8.4 — Message 元数据驱动的上下文控制（演化方向）

> 背景：在场景化 Agent 应用中（如角色扮演、NPC 对话），业务层需要向上下文注入非对话类消息（状态描述、旁白、世界观设定等），并控制这些消息在上下文管理中的行为（是否参与截断、摘要时是否保留等）。业务层的结构化状态最终都会被渲染为 Message，框架不需要理解业务状态的语义，只需要通过 Message metadata 识别控制意图。

设计要点：

- 复用现有 Message metadata（`HashMap<String, String>`），不新增控制指令字段，保持 Message 作为纯数据结构的职责单一性
- 框架约定一组保留的 metadata key 前缀（如 `lortex:`），用于上下文管理策略识别：
  - `lortex:pin` — 始终保留，不参与截断
  - `lortex:priority` — 摘要/截断时的优先级（high/normal/low）
  - `lortex:ephemeral` — 窗口滑动时可优先丢弃
- 业务层可自由使用其他 key（如 `app:narrator`、`app:state_injection`），框架忽略不识别的 key
- 提供类型安全的 helper 方法（如 `message.set_pinned()`、`message.is_pinned()`），底层仍读写 metadata
- 上下文管理策略（SlidingWindow、SummaryMemory 等）在执行截断/摘要时读取这些 metadata 做差异化处理
...
将底层框架组装为可独立运行的二进制代理应用。

#### 需求 11.1 — HTTP 服务网关

- 基于 `axum` 提供高性能 HTTP API 服务
- 暴露完美兼容 OpenAI API 规范的 `/v1/chat/completions` 和 `/v1/models` 端点
- 使得任何支持自定义大模型 API 的软件（如 Cursor, Windsurf, Cline 等）可以直接无缝接入

#### 需求 11.2 — 本地代理配置管理

- 支持通过本地配置文件（如 `config.toml` 或 `config.yaml`）管理不同 Provider 的 API Keys
- 支持在运行时通过修改配置文件实时更新路由规则，实现零停机热重载（Hot Reload）

#### 需求 11.3 — 基于 Router 的无缝分发

- 直接复用域三（模型管理与异构编排）的 `Router` 核心能力
- Proxy 代理层仅做协议解析与 API Key 验证，而核心的模型分发、日志追踪、成本记录直接下推给底层框架执行
...
> 目标：实现核心差异化能力，并推出 Lortex 的第一个产品应用形态（LLM Proxy Bin）
...
| 11.1-11.3 Proxy 服务 | 实现兼容 OpenAI API 的本地 HTTP 网关，集成配置管理 |
