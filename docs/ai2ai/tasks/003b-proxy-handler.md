...
## 任务分解
- [ ] T1: OpenAI 协议类型定义（ChatCompletionRequest/Response/StreamChunk）
  - 验证：序列化/反序列化与 OpenAI API 格式兼容
- [ ] T2: 协议转换层（OpenAI ↔ Lortex Message）
  - 验证：文本消息、tool call、system prompt 转换正确
- [ ] T3: API Key 鉴权中间件 + credit 扣减中间件
  - 验证：无效 key 返回 401，超额返回 429，正常请求后 credit 扣减
- [ ] T4: /v1/models handler
  - 验证：按 API Key 返回模型组，格式兼容 OpenAI /v1/models 响应
- [ ] T5: /v1/chat/completions handler（non-streaming）
