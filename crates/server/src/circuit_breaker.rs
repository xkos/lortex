...
use crate::models::health::{CircuitState, ProviderHealthStatus};
...
impl CircuitBreaker {
    ...
    /// 判断 provider 是否可用（允许请求通过）
    ///
    /// - Closed → true
    /// - HalfOpen → true（允许探测）
    /// - Open → 检查冷却期，过期则转为 HalfOpen 并返回 true
    pub async fn is_available(&self, provider_id: &str) -> Result<bool, StoreError> {
        let status = match self.store.get_health_status(provider_id).await? {
            Some(s) => s,
    ...
    /// 记录一次成功请求
    pub async fn record_success(&self, provider_id: &str) -> Result<(), StoreError> {
        let mut status = self
            .store
            .get_health_status(provider_id)
            .await?
            .unwrap_or_else(|| ProviderHealthStatus::new(provider_id));
...
mod tests {
    ...
    async fn initially_available() {
        ...
        assert!(cb.is_available("openai").await.unwrap());
    }

    #[tokio::test]
    async fn stays_closed_below_threshold() {
        let cb = test_cb().await;
        cb.record_failure("openai").await.unwrap();
        cb.record_failure("openai").await.unwrap();
    ...
    async fn success_resets_failures() {
        ...
        cb.record_failure("openai").await.unwrap();
        cb.record_failure("openai").await.unwrap();
        cb.record_success("openai").await.unwrap();
    ...
    async fn half_open_after_cooldown() {
        ...
        let status = cb.store.get_health_status("openai").await.unwrap().unwrap();
    ...
    async fn half_open_success_closes() {
        ...
        tokio::time::sleep(std::time::Duration::from_millis(1100)).await;
        cb.is_available("openai").await.unwrap(); // triggers HalfOpen
    ...
    async fn providers_independent() {
        ...
        assert!(!cb.is_available("openai").await.unwrap());
        assert!(cb.is_available("anthropic").await.unwrap());
    }
}
