...
impl From<&ApiKey> for ApiKeyResponse {
    fn from(k: &ApiKey) -> Self {
        ...
        Self {
            ...
            fallback_models: k.fallback_models.clone(),
...
pub async fn get(
    ...
    let key = state.store.get_api_key_by_id(&id).await
        .log_internal("get_api_key_by_id")?
...
pub async fn create(
    ...
    state.store.upsert_api_key(&api_key).await
        .log_internal("upsert_api_key (create)")?;
...
pub async fn update(
    ...
    let mut key = state.store.get_api_key_by_id(&id).await
        .log_internal("get_api_key_by_id (update)")?
        .ok_or(StatusCode::NOT_FOUND)?;
    ...
    state.store.upsert_api_key(&key).await
        .log_internal("upsert_api_key (update)")?;
    Ok(Json(ApiKeyResponse::from(&key)))
}
...
pub async fn reveal_key(
    ...
    let key = state.store.get_api_key_by_id(&id).await
        .log_internal("get_api_key_by_id (reveal)")?
        .ok_or(StatusCode::NOT_FOUND)?;
